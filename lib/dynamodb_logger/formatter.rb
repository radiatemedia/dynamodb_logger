...
module DynamodbLogger
  class Formatter < Logger::Formatter
      ...
      (message.respond_to?(:merge) ? message : {'message' => message}).inject({}) do |cleaned_hash, item|
        ...
        when Array
          if value.all {|v| Numeric === v}
            ...
            {'SS' => value.collect {|v| v.to_s}.select {|v| !v.blank?}} #string set
          end
        else
          {'S' => value.to_s} #string
        end

        cleaned_hash[key] = cleaned_value unless cleaned_value.values[0].blank?
    ...
    def call(severity, time, progname, msg)
      ...
      message[@timestamp_name || 'timestamp'] = {'N' => time.to_f}
      message['severity'] = {'S' => severity.to_s}
      message['pid'] = {'N' => $$.to_s}
      message['server_name'] = {'S' => @server_name || Socket::gethostname}
      message
    end
  end
end
